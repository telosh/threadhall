...
| **GCP threadhall-dev**（Cloud Build / Cloud Run 検証） | **Dockerfile.dev** | イメージ内で `next dev`（[cloudbuild.dev.yaml](../../cloudbuild.dev.yaml)）。ステージング向け |
| **GCP threadhall-prod**（本番） | [Dockerfile](../../Dockerfile) | `next build` の **standalone** + `node server.js`（[cloudbuild.yaml](../../cloudbuild.yaml)） |

**補足:** dev を本番に近い挙動にしたい場合は、threadhall-dev で **`cloudbuild.yaml` + `Dockerfile`** を実行する選択肢もある。

**Cloud Run と PORT:** `Dockerfile.dev` は **`PORT`** で待受（未設定時 3000）。Cloud Run の既定 `listen` は **`PORT`（例: 8080）**。**本番 Dockerfile** の `ENV PORT=3000` はランタイムで上書きされる。
