...
Next.js を **Dockerfile** でコンテナ化し（`next build` のあと **devDependencies を落とした `node_modules`** と **`.next`** を載せる）、**`npm run start`**（= [`next start --hostname 0.0.0.0`](../../package.json) … **`PORT` は Cloud Run が注入**）で **Artifact Registry** 経由 **Cloud Run** にデプロイします。DB は **Turso Cloud**（`TURSO_DATABASE_URL` / `TURSO_AUTH_TOKEN`）。

[`output: "standalone"`](https://nextjs.org/docs/app/api-reference/config/next-config-js/output) は **使わない**。イメージはやや大きくなるが、本番は **Next が想定する `next start` 経路**に統一する。
...
| **GCP threadhall-dev**（Cloud Build / Cloud Run 検証） | [Dockerfile](../../Dockerfile) | [cloudbuild.dev.yaml](../../cloudbuild.dev.yaml)。Image: **`cloud-run-source-deploy/threadhall/threadhall-dev`**。サービス名 **`threadhall-dev`**（Cloud Run でエクスポートした YAML と同じ）。
| **GCP threadhall-prod**（本番） | [Dockerfile](../../Dockerfile) | [cloudbuild.yaml](../../cloudbuild.yaml）。`npm run build` → ランタイム **`npm run start`**。 |

**補足:** dev / prod で **起動方式は同一**。差分は GCP プロジェクト・Turso・Secret・Artifact Registry のタグだけ。

**Cloud Run と PORT:** [package.json](../../package.json) の **`start`** は **`next start --hostname 0.0.0.0`**（[コンテナ契約](https://docs.cloud.google.com/run/docs/container-contract)の `0.0.0.0` 待受）。**待ちポートは環境変数 `PORT`**（Cloud Run が **例: 8080** を注入し、Dockerfile の `ENV PORT=3000` を上書き）。**ローカルの [Dockerfile.dev](../../Dockerfile.dev) のみ** `npm run dev`。

**スケール時のメモ:** Run はインスタンスが複数になる。**オンデマンド再検証やファイル系キャッシュに強く依存する構成**にすると、インスタンス間で挙動が割れることがある。現状の構成なら優先度は低いが、ISR を本格運用するときは [自己ホストのキャッシュ戦略](https://nextjs.org/docs/app/guides/self-hosting)も参照する。
...
gcloud builds submit --config cloudbuild.dev.yaml \
  --substitutions=SHORT_SHA="$(git rev-parse HEAD)"
```
...
**起動コマンド:** [cloudbuild.yaml](../../cloudbuild.yaml) / [cloudbuild.dev.yaml](../../cloudbuild.dev.yaml) の deploy には **`--command=npm`・`--args=run`・`--args=start`** を付け、**`npm run start` を固定**する（コンソールで `next dev` に上書きされたリビジョンを正す）。

### ログに `next dev` が出るとき

**あるべき姿:** Cloud Run（`threadhall-dev` / 本番とも）では **`npm run start`（`next start`）のみ**。`next dev` は **ローカルの [Dockerfile.dev](../../Dockerfile.dev) / compose だけ**。
...
2. Cloud Run の **コンテナ** で **コマンド・引数の上書き**が残っていないか。`npm run dev` 等なら削除し、再デプロイ（`npm run start` = 上記 YAML の `--command` / `--args`）に合わせる。
...
| ファイル | 役割 |
|----------|------|
| [Dockerfile](../../Dockerfile) | Cloud Run **dev / prod 共通**（`next build` + 本番依存のみ + `npm run start`） |
| [Dockerfile.dev](../../Dockerfile.dev) | **ローカル compose のみ**（`next dev`） |
| [cloudbuild.yaml](../../cloudbuild.yaml) | Cloud Build **prod** |
| [cloudbuild.dev.yaml](../../cloudbuild.dev.yaml) | Cloud Build **dev GCP**（`cloud-run-source-deploy`・`threadhall-dev` と整合） |
