...
## 参加トークン（FR-06 ・ [`docs/system/features-mvp.md`](../system/features-mvp.md)）

- **JWT はクレームを最小化**して文字列長を抑える（例: `event_id` / `jti` / `exp` のみ。メタデータはサーバ或いは別テーブル）。
- QR は **短命トークンそのもの**か **短い参照キー＋サーバ引き**の二択をスパイクで決める（[`docs/system/frozen-decisions.md`](../system/frozen-decisions.md) F-06）。
