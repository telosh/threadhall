...
### 14.3.1 一次ソース（Google AI Studio mock）

Stitch MCP は cross-origin の iframe で操作できないため、**Stitch のトークン定義を完全継承した動作する React モック**を Google AI Studio で生成し、`docs/stich/aistudio/` に保存している。**実装の正はここ**。

- ファイル一覧: `docs/stich/aistudio/src/{index.css,App.tsx,types.ts,constants.ts,components/*.tsx}`
- 5 つの View が 1 アプリで切替: `dashboard` / `organization` / `thread` / `event-log` / `components`
- 使用ライブラリ: Tailwind v4（`@theme` ディレクティブ）、`lucide-react`、`motion/react`
- トークン定義は `docs/stich/aistudio/src/index.css` の `@theme` ブロックが SSOT。本リポジトリの `src/app/globals.css` は**これを完全反映する**

Stitch 側の HTML（`docs/stich/{community,eventlog,threadshousai}.tsx`）は AI Studio mock とトークン互換だが、**画面間で命名・余白・角丸・アイコン FILL の細かい不整合がある**。詳細は `docs/system/components.md` の「Stitch 不整合と統一ルール」節に集約済み。実装は **AI Studio mock を上書きしない統一ルールに従う**。
...
| 観点 | Stitch / AI Studio（UI） | 現リポジトリ（機能・UI） |
|------|----------------|---------------------------|
| カラーモード | **ライト**基調（surface `#f9f9f9`） | `globals.css` は §14.4 と整合 |
| フォント | Geist / Inter / JetBrains Mono | `layout.tsx` で同 3 種を読み込み |
| 画面 | Dashboard / Organizations / Threads / Events / Components の 5 画面（§14.3.1） | **本番ルートで実装済み**（mock データ）：`/dashboard`・`/orgs/[slug]`・`/threads/[threadId]`・`/events/[eventId]`・`/system/components`。各ページ仕様は [`docs/pages/`](pages/) |
| データ | mock データ（`docs/stich/aistudio/src/constants.ts`） | UI は `src/lib/mock-data.ts` を参照。**実 DB 接続は別 PR**（FR-01〜03 issue 配下）で `src/server/queries` から差し替える |
| `/` ルート | — | 開発スキャフォールドは `/dev` に退避、`/` は `/dashboard` へリダイレクト |
...
1. **本番データ接続**: 各ページ（`docs/pages/*.md`）の「データソース」節に `src/server/queries/*` を割り当て、mock を順次差し替え。FR-01/02/03 の Issue と同期。
2. **書込ゲートの UI 反映**: イベント `archived` 時の compose 無効化バナー、未ログイン時の sign-in CTA、トークンゲストの表示を `docs/pages/` の「ロール別見え方」と整合させる。
3. **モバイル**: AI Studio mock の `App.tsx` にある `md:hidden` の bottom nav を、本番でも同じ条件で出す。詳細は `docs/system/components.md`。
4. **Stitch との不整合解消**: `docs/system/components.md` の「Stitch 不整合と統一ルール」を逐次更新。新画面を Stitch から取り込むときはここに追記。
5. **Components ページの拡張**: `docs/stich/aistudio/src/components/ComponentsView.tsx` は最小限。phase バッジ・avatar・badges・state ごとのバリエーションを追加して**実装の単一参照**にする。
